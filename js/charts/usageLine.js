...
export function renderUsageLine(data) {
  ...
  el.selectAll('*').remove();
  const W = el.node().clientWidth || 560, H = 280, M = { t: 18, r: 52, b: 36, l: 48 };
  const svg = el.append('svg').attr('width', '100%').attr('height', H).attr('viewBox', `0 0 ${W} ${H}`);

  // Only months present in data
  ...
  legend.append('span').html(`<i style="background:${C.blue}"></i> Avg AI hours / week (right axis, dashed)`);
}