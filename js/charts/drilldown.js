...
import { themeColors, majorColorScale, showTip, hideTip } from '../utils/helpers.js';
...
export function renderDrilldown(data) {
  ...
  const W = el.node().clientWidth || 900, H = 280, M = { t: 14, r: 16, b: 44, l: 44 };
  ...
  const x = d3.scaleBand().domain(agg.map(keyFn)).range([M.l, W - M.r]).padding(0.3);
  const y = d3.scaleLinear().domain([0, d3.max(agg, d => d[1]) || 1]).nice().range([H - M.b, M.t]);
  ...
  svg.selectAll('rect').data(agg).enter().append('rect')
    .attr('x', d => x(keyFn(d))).attr('width', x.bandwidth()).attr('y', d => y(d[1])).attr('height', d => y(0) - y(d[1])).attr('rx', 3)
    ...
    .on('click', (evt, d) => { if (!state.drillYear) { state.drillYear = d[0]; if (window.__refreshAll) window.__refreshAll(); } });
