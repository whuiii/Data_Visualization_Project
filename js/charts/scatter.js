...
export function renderScatter(fullData, sampledData) {
  ...
  const x = d3.scaleLinear().domain([0, d3.max(allData, d => d.Weekly_GenAI_Hours) + 1]).range([M.l, W - M.r]);
  const y = d3.scaleLinear().domain([d3.min(allData, d => d.Post_Semester_GPA) - 0.1, d3.max(allData, d => d.Post_Semester_GPA) + 0.1]).range([H - M.b, M.t]);
  const r = d3.scaleLinear().domain(d3.extent(allData, d => d.Tool_Diversity)).range([2.6, 7.5]);
  ...
  // Brush (still works on the chart area)
